...
def settle_stream_payment_after_end(session: Session, stream: Stream) -> dict:
    """
    Called when broadcaster ends stream.

    If stream duration is valid:
        transfer each transaction separately.

    If stream duration is too short:
        refund each paid viewer separately.
    """
